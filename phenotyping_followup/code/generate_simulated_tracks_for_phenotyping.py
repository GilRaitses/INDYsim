...
Uses the validated INDYsim trajectory simulator to generate complete 10-minute tracks
...
FIXED (2025-12-21):
- Duration: 10 minutes (600 seconds) to match Methods section
- Intercept: -6.54 (from parameter sweep, achieves 1.55 events/min)
- Track intercept std: 0.38 (from parameter sweep)
"""
...
def generate_phenotype_tracks(
    n_tracks: int = 300,
    duration: float = 600.0,  # 10 minutes (FIXED: was 20 min)
    condition: str = '0-250_Constant',
    phenotype_variation: bool = True,
    track_intercept_std: float = 0.38,
    ...
    FIXED (2025-12-21):
    - Duration: 600 seconds (10 minutes) to match Methods section
    - Intercept: -6.54 (from parameter sweep, achieves 1.55 events/min)
    - Track intercept std: 0.38 (from parameter sweep)
    """
    print("=" * 70)
    ...
    # FIXED (2025-12-21): intercept from parameter sweep optimization
    # Parameter sweep found: intercept=-6.54, track_intercept_std=0.38
    # Achieves rate: 1.55 events/min (target: 1.546 events/min, error: 0.004)
    base_params = KernelParams(
        ...
        D=-0.114,
        tau_off=2.0,
        intercept=-6.54  # FIXED: From parameter sweep (optimal value)
    )
    ...
    for track_id in range(1, n_tracks + 1):
        ...
        # FIXED: Must include intercept from base_params
        track_params = KernelParams(
            ...
            intercept=base_params.intercept  # FIXED: Use corrected intercept
        )
        ...
        # Generate event times BEFORE simulation (to count correctly)
        event_times = model.simulate_events_discrete(
            duration=duration,
            led_onset_times=led_onsets,
            led_offset_times=led_offsets,
            track_intercept=track_intercept,
        ...
        # FIXED: Count actual reorientation events, not turn frames
...
def generate_multi_condition_dataset(
    ...
    duration: float = 600.0,  # FIXED: 10 minutes (was 20 min)
    phenotype_variation: bool = True,
    seed: int = 42,
    output_dir: Path = None
) -> pd.DataFrame:
