...
def simulate_doe_condition(models_dict, condition, config, empirical_data=None,
    ...
    for rep in range(n_replications):
        ...
        try:
            ...
            error_count += 1
            rep_elapsed = time.time() - rep_start_time
            print(f"    FAIL Replication {rep + 1}/{n_replications} FAILED ({rep_elapsed:.1f}s): {e}")
            sys.stdout.flush()
    ...
    if error_count > 0:
        print(f"    WARNING {error_count} replication(s) failed")
    sys.stdout.flush()
...
def main():
    ...
    if event_params_file.exists():
        ...
        print(f"\nWARNING  Event parameters not found at {event_params_file}")
            ...
            if os.name == 'posix':  # macOS/Linux
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        print(f"WARNING  Could not auto-launch monitor (exit code {result.returncode})")
                        if result.stderr:
                            print(f"   Error: {result.stderr[:200]}")
                        print(f"   Please manually run: python3 {monitor_script}")
                except subprocess.TimeoutExpired:
                    print("WARNING  Monitor launch timed out")
                    print(f"   Please manually run: python3 {monitor_script}")
                except Exception as e:
                    print(f"WARNING  Error launching monitor: {e}")
                    print(f"   Please manually run: python3 {monitor_script}")
            else:
                print("WARNING  Auto-launch not supported on this OS")
                print(f"  Please manually run: python3 {monitor_script}")
        except Exception as e:
    ...
    if run_full_doe:
        if not args.force_full_doe:
            ...
            print("\nWARNING  This will generate a large dataset.")
    ...
    if len(conditions_to_run) == 0:
        ...
        for idx, (_, condition_row) in enumerate(conditions_to_run.iterrows()):
            ...
            try:
                ...
                total_errors += 1
                print(f"  FAIL Condition {condition_id} FAILED: {e}")
                sys.stdout.flush()
                
                if args.continue_on_error and total_errors < args.max_errors:
                    print(f"  WARNING Continuing (error {total_errors}/{args.max_errors})")
                    continue
                else:
