...
def calculate_stimulus_locked_turn_rate_from_data(events_df, cycles, h5_file=None):
    ...
    2. For each cycle, align times relative to LED ON (cycle time 0)
    ...
    # Extract reorientation start times directly (MATLAB-style approach)
    # This avoids issues with double-binning and aggregation loss from summing boolean values
    # NOTE: Events CSV uses 50ms bins with 'any' aggregation, so if multiple reorientations
    # start in the same 50ms bin, they may be counted as one. This is rare in practice.
    # For maximum accuracy, consider using trajectories CSV (frame-level resolution) instead.
    reorientation_events = events_df[events_df['is_reorientation'] == True].copy()
    
    if len(reorientation_events) == 0:
        print("  Warning: No reorientation events found in events CSV")
        return None
    ...
    for cycle in cycles:
        ...
        cycle_reorientations['time_rel_onset'] = cycle_reorientations['time'] - led_on_time
