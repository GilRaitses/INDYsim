...
$eti$ & array & Experiment Time Index (REQUIRED for time calculation, loaded from H5 root) \\
$fps$ & float & Frames per second (frame rate) - ONLY for pulse duration, NOT for time arrays \\
...
\STATE \textcolor{red}{CRITICAL: Verify ETI loaded: $eti \leftarrow h5\_data[\text{eti}]$ (raise ValueError if missing)}
\STATE Extract metadata: $fps \leftarrow h5\_data[\text{metadata}][\text{fps}]$
\STATE Extract stimulus timing: $stimulus\_df \leftarrow \text{extract\_stimulus\_timing}(h5\_data)$ \textcolor{red}{(uses ETI internally)}
...
\STATE Extract trajectory features: $df \leftarrow \text{extract\_trajectory\_features}(track\_data, fps, eti=h5\_data[\text{eti}])$ \textcolor{red}{(ETI REQUIRED)}
...
\begin{algorithmic}[1]
\REQUIRE Track data $track\_data$, frame rate $fps$, ETI array $eti$ (REQUIRED)
\ENSURE Trajectory DataFrame $df$ with features

\STATE \textcolor{red}{CRITICAL: Verify ETI is provided (raise ValueError if None)}
\STATE Load raw data: $x, y, heading, speed, curvature \leftarrow track\_data$
\STATE \textcolor{red}{Calculate time: $time \leftarrow eti.copy()$ (NOT $np.arange(n\_frames) / fps$)}
...
\STATE \textcolor{red}{Validate: If $\max(time) > 1200$ seconds, raise ValueError (experiments are 20 min max)}
...
## Time Calculation - CRITICAL ERROR FIXED

\textcolor{red}{\textbf{CRITICAL ERROR FIXED: ETI MUST ALWAYS BE USED FOR TIME CALCULATION}}

\textcolor{red}{Using frame-based time calculation (\texttt{np.arange(n\_frames) / frame\_rate}) is FORBIDDEN and causes tracks to exceed 20 minutes, which is physically impossible.}

\textcolor{red}{\textbf{Error:}}
\begin{itemize}
    \item \textcolor{red}{Experiments are only 20 minutes long (1200 seconds)}
    \item \textcolor{red}{Using \texttt{time = np.arange(n\_frames) / frame\_rate} assumes continuous frames}
    \item \textcolor{red}{This produces durations of 37+ minutes for tracks with gaps}
    \item \textcolor{red}{Example: Track with 22,722 frames at 10 Hz = 2272.2 seconds = 37.9 minutes (IMPOSSIBLE)}
\end{itemize}

\textcolor{red}{\textbf{Correct Implementation:}}
\begin{itemize}
    \item \textcolor{red}{ETI (Experiment Time Index) MUST be loaded from H5 root level}
    \item \textcolor{red}{ETI MUST be passed to all functions that calculate time}
    \item \textcolor{red}{Use \texttt{time = eti.copy()} or \texttt{time = eti[track\_frame\_indices]} for time arrays}
    \item \textcolor{red}{frame\_rate is ONLY used for pulse duration calculations, NOT for time arrays}
\end{itemize}

\textcolor{red}{\textbf{Policy:}}
\begin{itemize}
    \item \textcolor{red}{See \texttt{docs/ETI\_TIME\_CALCULATION\_POLICY.md} for complete policy}
    \item \textcolor{red}{All functions MUST raise ValueError if ETI is missing}
    \item \textcolor{red}{All time calculations MUST validate that time does not exceed 1200 seconds}
    \item \textcolor{red}{Functions that calculate time MUST accept ETI parameter}
\end{itemize}

\textcolor{red}{\textbf{Functions Fixed:}}
\begin{itemize}
    \item \textcolor{red}{\texttt{extract\_trajectory\_features()} - Now requires ETI parameter}
    \item \textcolor{red}{\texttt{extract\_stimulus\_timing()} - Now uses ETI from h5\_data}
    \item \textcolor{red}{\texttt{load\_h5\_file()} - Now loads ETI from root level}
\end{itemize}
