---
title: "Interface Dynamics Simulation (InDy-sim)"
subtitle: "Stimulus-Driven Behavioral Modeling of Drosophila Larvae"
author: "Gil Raitses"
date: today
abstract: |
  \tech{Interface Dynamics Simulation (InDy-sim)} investigates behavior as the \tech{interface between perception and adaptation}, combining \tech{agent-based simulation} with \tech{dynamic transition modeling}. This project develops a \tech{stimulus-locked event-hazard model} for \tech{Drosophila larval behavior}, treating \tech{behavioral transitions} as both signal and medium that reveal how \tech{sensory representation} translates into \tech{neural activity} and observable behavior. The model estimates time-varying hazard rates for behavioral events conditioned on \tech{stimulus intensity}, \tech{temporal history}, and \tech{contextual features} including speed and orientation. Using a \tech{design of experiments} framework with \tech{stimulus intensity}, \tech{pulse duration}, and \tech{inter-pulse interval} as factors, the analysis generates \tech{simulated trajectories} that produce \tech{key performance indicators} including mean turn rate, latency to first turn, stop fraction, and spatial distribution metrics. \tech{Confidence intervals} for behavioral metrics provide statistical precision comparable to \tech{Arena simulation replications}, which supports validation of model predictions against empirical larval trajectory data.
format:
  pdf:
...
\tech{Interface Dynamics Simulation (InDy-sim)} investigates behavior as the \tech{interface between perception and adaptation}, treating behavioral transitions as both signal and medium. This framework bridges \tech{computational neuroscience} with \tech{media theory}, modeling how \tech{sensory representation} translates into \tech{neural activity} and ultimately manifests as observable behavior. The behavioral interface reveals how organisms process time-varying stimuli through \tech{hierarchical dynamics} spanning sensory transduction, neural integration, and motor output.
...
Using raised cosine basis functions (see Equation \ref{eq:kernel} in Appendix) with knots $\tau_j$ spanning $[-2, 20]$ seconds relative to stimulus onset.

### Event Likelihood and Estimation

For larva $i$ with observed events at times $\{t_{i,k}\}$, the log-likelihood follows Equation \ref{eq:likelihood} in Appendix. Estimation via \tech{penalized maximum likelihood} with $L_2$ regularization (see Equation \ref{eq:estimation} in Appendix).
...
Run speeds follow observed empirical distributions (typically log-normal). During runs, integrate using Equation \ref{eq:speed_dynamics} in Appendix where $v(t)$ is current speed and $\theta(t)$ is current heading.
...
**Full factorial design**: $3 \times 5 \times 3 = 45$ conditions. For each condition, the simulation runs \num{30} replications where each replication represents one larva. The process records events and computes KPIs per replication, then generates \tech{AcrossReplicationsSummary.csv} with means and confidence intervals.

### Run Length Determination

The replication length follows Lab04 CI methodology. Target precision uses \num{10}\% relative half-width for mean turn rate, which means when the mean equals \num{5} turns per minute the confidence interval width stays below \num{0.5} turns per minute. A pilot study runs \num{10} replications to estimate variance $\hat{\sigma}^2$. Required replications follow Equation \ref{eq:run_length} in Appendix where $E$ represents the desired half-width. When $n > 30$ the simulation uses that calculated value, while values $n \leq 30$ default to \num{30} replications as the minimum standard.
...
The primary data source uses H5 files containing exported MAGAT experiment data in HDF5 format. These files contain trajectory positions, LED stimulus data, and metadata processed using the feature extraction pipeline. Backup sources include trajectory CSVs for runs and reorientations, stimulus CSVs with frame-level LED values, and experimental metadata with experiment IDs, conditions, and replication numbers.

### Feature Extraction

**Step 1: Extract from H5 Files**

The extraction command is shown in Code \ref{code-h5-extract} in Appendix. This creates three output files: event records with 50ms time bins, full trajectory data aligned with stimulus, and summary statistics in CSV and JSON formats.

**Step 2: Feature Engineering** (implemented in `engineer_dataset_from_h5.py`)

The feature engineering pipeline is detailed in Code \ref{code-feature-engineering} in Appendix.
...
The simulation engine implementation is shown in Code \ref{code-simulator} in Appendix.

### DOE Execution

For each of 45 conditions, the DOE execution follows Code \ref{code-doe-execution} in Appendix.

## Statistical Analysis

### Performance Metrics Calculation

For each KPI and condition, the analysis calculates four statistics. The mean uses $\bar{X} = \frac{1}{n}\sum_{i=1}^n X_i$. Standard deviation follows $s = \sqrt{\frac{1}{n-1}\sum_{i=1}^n (X_i - \bar{X})^2}$. The 95\% confidence interval uses $\bar{X} \pm t_{0.025,n-1} \frac{s}{\sqrt{n}}$. Minimum and maximum values track $\min_i X_i$ and $\max_i X_i$ (see Equations \ref{eq:mean}--\ref{eq:ci} in Appendix).
...
**Equation \ref{eq:hazard}: Hazard Rate Model**
...
**Equation \ref{eq:kernel}: Temporal Kernel Basis Expansion**

The temporal kernel uses raised cosine basis functions centered at knots $\tau_j$ spanning the analysis window. Each basis function has local support $[\tau_j-\Delta\tau, \tau_j+\Delta\tau]$ and captures stimulus-response dynamics at different time delays relative to stimulus onset.
...
**Equation \ref{eq:likelihood}: Event Likelihood**
...
**Equation \ref{eq:estimation}: Penalized Maximum Likelihood Estimation**

Parameter estimation maximizes the log-likelihood subject to $L_2$ regularization penalty. The regularization parameter $\lambda_{\text{reg}}$ controls the trade-off between model fit and complexity, preventing overfitting to training data.
...
**Equation \ref{eq:speed_dynamics}: Position Integration**

During run phases, position updates using forward Euler integration with current speed $v(t)$ and heading angle $\theta(t)$. The integration step size $\Delta t$ balances numerical accuracy with computational efficiency.

\begin{equation}
\label{eq:speed_dynamics}
\mathbf{x}(t+\Delta t) = \mathbf{x}(t) + v(t) \Delta t \cdot [\cos(\theta(t)), \sin(\theta(t))]^\top
\end{equation}

**Equation \ref{eq:run_length}: Replication Count Determination**

The required number of replications $n$ depends on the desired confidence interval half-width $E$, estimated variance $\hat{\sigma}^2$, and significance level $\alpha$. This formula ensures statistical precision targets are met while minimizing computational cost.

\begin{equation}
\label{eq:run_length}
n \geq \left(\frac{z_{\alpha/2} \hat{\sigma}}{E}\right)^2
\end{equation}

### Statistical Metrics

**Equation \ref{eq:mean}: Sample Mean**
...
**Equation \ref{eq:std}: Sample Standard Deviation**
...
**Equation \ref{eq:ci}: 95\% Confidence Interval**
...
**Code \ref{code-h5-extract}: H5 Data Extraction Command**
...
**Code \ref{code-feature-engineering}: Feature Engineering Pipeline**
...
**Code \ref{code-simulator}: Trajectory Simulator Class**
...
**Code \ref{code-doe-execution}: Design of Experiments Execution Loop**
