...
model_summary <- data.frame(
  Event_Type = c("Reorientation", "Pause", "Reversal"),
  Baseline_Rate = c(
    sprintf("%.2f turns/min", as.numeric(baseline_reorientation$rate_per_min)),
    sprintf("%.1f pauses/min", as.numeric(baseline_pause$rate_per_min)),
    sprintf("%.1f reversals/min", as.numeric(baseline_reversal$rate_per_min))
  ),
  N_Features = c("10 kernel basis", "10 kernel basis", "10 kernel basis"),
  Validation = c("KS test, PSTH", "KS test, PSTH", "KS test, PSTH")
)

kable(model_summary,
      caption = "Fitted Hazard Model Summary",
      format = "html",
      booktabs = TRUE,
      col.names = c("Event Type", "Baseline Rate", "Kernel Features", "Validation")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```

All three event types were successfully modeled with temporal kernels capturing stimulus-locked responses.

### Temporal Kernel Visualization

![Reorientation Kernel showing stimulus-locked response dynamics](output/figures/kernel_reorientation.png){width=80%}

The kernel captures latency, peak response, and decay phases relative to stimulus onset.

### DOE Results: Overview

```{r doe-overview, echo=FALSE}
summary_df <- read_csv("output/analysis/arena_csvs/AcrossReplicationsSummary.csv", show_col_types = FALSE)

doe_overview <- data.frame(
  Metric = c("Total Conditions", "Replications per Condition", "Total Simulations", 
             "Mean Turn Rate", "Turn Rate Range", "Typical CI Half-Width"),
  Value = c(
    "45",
    "30",
    "1,350",
    sprintf("%.2f turns/min", mean(summary_df$TurnRate_Mean, na.rm = TRUE)),
    sprintf("%.2f - %.2f turns/min", 
            min(summary_df$TurnRate_Mean, na.rm = TRUE),
            max(summary_df$TurnRate_Mean, na.rm = TRUE)),
    sprintf("%.2f turns/min", mean(summary_df$TurnRate_HalfWidth, na.rm = TRUE))
  )
)

kable(doe_overview,
      caption = "DOE Execution Summary",
      format = "html",
      booktabs = TRUE,
...
### Main Effects on Turn Rate

![Main effects showing how intensity, pulse duration, and inter-pulse interval affect turn rate](output/analysis/figures/doe_analysis/main_effects_TurnRate.png){width=90%}

Threshold intensity (25%) produces lower turn rates compared to higher intensities. Pulse duration shows systematic effects across conditions.

### Top Performing Conditions

```{r top-conditions, echo=FALSE}
top_conditions <- summary_df %>%
  arrange(desc(TurnRate_Mean)) %>%
  head(5) %>%
  select(ConditionID, Intensity, PulseDuration, InterPulseInterval, TurnRate_Mean, TurnRate_HalfWidth) %>%
  mutate(
    TurnRate_CI = sprintf("%.2f ± %.2f", TurnRate_Mean, TurnRate_HalfWidth)
  ) %>%
  select(ConditionID, Intensity, PulseDuration, InterPulseInterval, TurnRate_CI)

kable(top_conditions,
      caption = "Top 5 Conditions by Turn Rate",
      format = "html",
      booktabs = TRUE,
      col.names = c("Condition", "Intensity (%)", "Pulse (s)", "IPI (s)", "Turn Rate (95% CI)")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
1. **Model Performance**: Successfully captured stimulus-locked responses with biologically plausible event rates
2. **DOE Precision**: 30 replications per condition provided tight confidence intervals (±0.2-0.3 turns/min)
3. **Factor Effects**: Intensity and pulse duration showed systematic effects on behavioral responses
4. **Statistical Rigor**: Arena-style analysis enabled comparison across 45 experimental conditions

### Course Integration

This project demonstrates application of ECS630 concepts:

- **Input characterization**: Stimulus distributions and behavioral state initialization
- **Replication analysis**: Sequential estimation to determine sample size
- **Output diagnostics**: KPI distributions and confidence intervals
- **DOE methods**: Factorial design with statistical analysis
