...
Model performance successfully captured stimulus-locked responses with biologically plausible event rates. DOE precision achieved 30 replications per condition providing tight confidence intervals (±0.2-0.3 turns/min). Factor effects showed intensity and pulse duration with systematic effects on behavioral responses. Statistical rigor enabled Arena-style analysis comparison across 45 experimental conditions.

### Course Integration

This project demonstrates application of ECS630 concepts: input characterization (stimulus distributions and behavioral state initialization), replication analysis (sequential estimation to determine sample size), output diagnostics (KPI distributions and confidence intervals), and DOE methods (factorial design with statistical analysis).
